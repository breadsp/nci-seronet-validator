...
import pandas as pd                             #pandas library to create datatables
...
def get_mysql_queries(file_dbname,conn,index):
    ...
    elif index == 2:  ## list of Assay IDs and Assay Targets, Used to validate Assay Target and Confimatory Clinical
    ...
    elif index == 3: ## list of Biospecimen_ID and Biosepciemnt Types, used to validate Aliquot,Equipment, Reagent and Consumable
...
class Submitted_file:  
    ...
    def get_csv_table(self,file_name):           #loads the data from a file into a pandas data frame
...
##unique to prior test 
    def remove_unknown_sars_results(self):      #if a Participant has a SARS_Cov2_PCR test that is not positive or negative, write an error and remove from table
    ...
    def remove_unknown_sars_results_v2(self): #if a Participant has a SARS_Cov2_PCR test that is not positive or negative, write an error
        ...
        self.Data_Table = self.Data_Table[(self.neg_list_logic | self.pos_list_logic)]          #Filter table to only Positive/Negative Participants
...
###############################################################################         
    ...
    def is_string(self,column_name,test_value,na_allowed,error_msg,row_number,error_stat):              #see if the value is a string (check for initals)
        if (na_allowed == True) & ((test_value != test_value) | (test_value == "N/A")):       #value is N/A
            ...
            try:
                float(test_value)       #value is a number, return error
                self.write_error_msg(test_value,column_name,error_msg,row_number,error_stat)
            except:
                try: 
    ...
    #def write_error_file(self,file_name):    #writes out the error list to a file
    def write_error_file(self,file_name,s3_resource,s3_file_path,output_bucket_name):
